...
TEST_CASE("fixed flat set" * test_suite("all")) {
  ...
  {
    pmr_vector_flat_set<int> v{1,2,3};
    v.insert(v.begin(), 5);
    REQUIRE(v == pmr_vector_flat_set<int>{1, 2, 3, 5});
    v.insert(v.begin(), 5);
    REQUIRE(v == pmr_vector_flat_set<int>{1, 2, 3, 5});
    v.insert(v.begin() + 3, 4);
    REQUIRE(v == pmr_vector_flat_set<int>{1, 2, 3, 4, 5});
  }
  {
    pmr_vector_flat_set<int> v{1,2,3};
    v.erase(2);
    REQUIRE(v == pmr_vector_flat_set<int>{1, 3});
  }
  {
    pmr_vector_flat_set<int> v{1,2,4};
    auto iter = v.lower_bound(3);
    REQUIRE(iter == v.begin() + 2);
    v.insert(iter, 3);
